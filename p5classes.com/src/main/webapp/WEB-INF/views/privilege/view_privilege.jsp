<%@ include file="../static/common_taglib.html"%>
<!DOCTYPE html>
<html lang="en">
<head>
<title>View Privilege</title>
<%@ include file="../static/common_style_script.html"%>
</head>
<body>
	<!-- Top Menu, Side Menu -->
	<%@include file="../static/menu.html"%>
	<!-- Top Menu, Side Menu End -->

	<!-- Main Content -->
	<div class="w3-main" id="mainContent">
	<%@include file="../static/header_menu.html"%>
	
		<div class="formStyle">
			<Spring: type="hidden" path="privilegeId" id="privilegeId" />
			<table style="margin: 0 auto;">
				<tr><td colspan=3 class="formHeading">Privilege Details</td></tr>
				<tr><td colspan=3>${SUCCESS_MSG} &nbsp;</td></tr>

				<tr>
					<td><spring:message code="label.privilege.privId" />:</td>
					<td>${privilege.privId}</td>
				</tr>
			
				<tr>
					<td><spring:message code="label.privilege.privName" />:</td>
					<td>${privilege.privName}</td>
				</tr>
				<tr>
					<td><spring:message code="label.privilege.details" />:</td>
					<td>${privilege.details}</td>
				</tr>
				<tr>
					<td><spring:message code="label.privilege.mappedPath" />:</td>
					<td>${privilege.mappedPath}</td>
				</tr>
				<tr>
					<td><spring:message code="label.privilege.pageId" />:</td>
					<td>${privilege.pageId}</td>
				</tr>


				<tr>
					<sec:authorize access="hasRole('DBA')">
						<td colspan=2>
							<a href="<c:url value='/privilege/addPrivilege' />">Add</a>|
							<a href="<c:url value='/privilege/editPrivilege/${privilege.privId}' />">edit</a>
							<a href="<c:url value='/privilege/deletePrivilege/${privilege.privId}' />">delete</a>
						</td>
					</sec:authorize>
				
					<sec:authorize access="hasRole('ADMIN')">
						<td colspan=2>
							<a href="<c:url value='/privilege/addPrivilege' />">Add</a>|
							<a href="<c:url value='/privilege/editPrivilege/${privilege.privId}' />">edit</a>
							<a href="<c:url value='/privilege/deletePrivilege/${privilege.privId}' />">delete</a>
						</td>
					</sec:authorize>
					<sec:authorize access="hasRole('USER')">
						<td colspan=2>

						</td>
					</sec:authorize>
				</tr>
			</table>
		</div>

		<!-- Footer -->
		<%@include file="/static/footer.html"%>
		<!-- Footer End-->
	</div>
	<!-- Main Content End -->
</body>
</html>