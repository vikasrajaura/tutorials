...
@Repository("userDao")
public interface IUserDao extends JpaRepository<UserDetails, Long> {
	...
	List<UserDetails> findByOrg(@Param("orgId") Long orgId);
	
	@Query(value = "SELECT * FROM USERS WHERE BRANCH_ID = :branchId", nativeQuery = true)
	List<UserDetails> findByBranch(@Param("branchId") Long branchId);
